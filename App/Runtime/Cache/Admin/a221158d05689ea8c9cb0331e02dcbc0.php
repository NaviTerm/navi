...
<form action="<?php if($list): echo U('admin/mall/update'); else: echo U('admin/mall/add'); endif; ?>" method="post" enctype="multipart/form-data">
<?php if($list[itemid]): ?><input type="hidden" name="itemid" value="<?php echo ($list['itemid']); ?>" /><?php endif; ?>
    <ul class="forminfo">

    <li><label>标题<b>*</b></label><input name="title" type="text" class="dfinput" value="<?php echo ($list[title]); ?>"  maxlength="30"  placeholder="请填写标题" style="width:576px;" required="required"/></li>
    ...
    <option value="1" <?php if($list[level] == 1): ?>selected<?php endif; ?>>一级</option>
    <option value="2" <?php if($list[level] == 2): ?>selected<?php endif; ?>>二级</option>
    <option value="3" <?php if($list[level] == 3): ?>selected<?php endif; ?>>三级</option>
    </select>
    </div>
    </li>
    ...
    <li>
    	<label>简介</label>
    	<textarea name="introduce" style="border:1px solid #ccc" cols="80" rows="8"><?php echo ($list[introduce]); ?></textarea>
    </li>

    <li><label>产品内容<b>*</b></label>
...
" /></li>
	...
	<li>
		<label>内容模版</label>
		<div class="vocation">
			<select name="template" class="select1">
