...
.imgtable tr td{
    ...
    <div class="tools">
    	...
    	<ul class="toolbar">
        <li onclick="location='<?php echo U('admin/link/add');?>'">
            <span><img src="<?php echo (AD_SKIN); ?>images/t01.png" /></span>添加友链
        </li>
        <li class="form_seva">
...
<form method="post" id="form_seva" action="<?php echo U('link/update_seva');?>">
    <table class="imgtable">
        <thead>
            <tr>
                <th width="100"><input type="checkbox" id="Qx" /></th>    
                <th width="150">排序</th>
                <th width="150">分类</th>
                <th width="150">等级</th>
                <th>标题</th>
        ...
        <?php if(is_array($lists)): foreach($lists as $key=>$t): ?><tr>
                <td width="100">
                    <input type="checkbox" name="<?php echo ($t[itemid]); ?>" value="<?php echo ($t[itemid]); ?>">
                </td>    
                <td class="imgtd" width="150">
                    <input type="text" size="2" name="<?php echo ($t[itemid]); ?>" value="<?php echo ($t[listorder]); ?>">
                </td>
                <td width="150">
                    <?php echo (get_typeid_name($t[typeid])); ?>
                </td>
                <td width="150">
                    <?php echo (get_level_name($t[level])); ?>
                </td>
                <td>
                    <?php echo ($t[title]); ?>
                    <p>添加时间：<?php echo (date('Y-m-d H:i:s',$t[addtime])); ?></p>
                </td> 
                <td>
                    <a href="<?php echo U('admin/link/update',array('itemid'=>$t[itemid]));?>">编辑</a>&nbsp;&nbsp;
                    <a href="<?php echo U('admin/link/delete',array('itemid'=>$t[itemid]));?>">删除</a>
                </td>
