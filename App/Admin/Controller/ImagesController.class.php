...
*/
		...
		public function imgup_png($formname,$id){
...
/*					 $images->width($width);
				     ...
				     // $images->thumb($width,$height,2);  //设置处理后图片的高度宽度 1等比缩放 2裁剪填充
			         //$thumb = PUB.$result['savepath'].$result['savename'].'.thumb.'.$result['ext'];//缩略图保存路径
		             //$thumb_info=$images->save('.'.$thumb,'png','100');
		             $thumb_info = PUB.$result['savepath'].$result['savename'];
		          	 if( $thumb_info ){
			          	 //unlink($imgpath);//删除原图
			           	 echo "<script>parent.callback('".$thumb_info."',true,'".$id."')</script>"; 	
		           	}else{
		           		 echo "<script>parent.callback('".$thumb_info."',false,'".$id."')</script>";
		           	}
					/*缩略图结束*/
				}else{
					return $upload->getError();
